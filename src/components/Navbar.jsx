...
function Navbar() {
  ...
  return (
    <div className='fixed top-[calc(100vh-100px)] lg:top-0 lg:left-0 z-[99]'     >
        ...
        <div className="p-4  group w-screen lg:w-[120px] h-[100px] lg:h-screen text-white bg-[#0f1013] lg:bg-transparent   flex lg:flex-col justify-center center-mobile  lg:gap-6 gap-14 transition-[width] duration-500 lg:hover:bg-gradient-to-r from-#0f1013 to-gray-400 lg:hover:w-[20vw]"
        style={{ backgroundImage: `linear-gradient(to right, #0f1013 , transparent)` }}
         ...
         >
            <div  className={` cursor-pointer lg:hover:scale-[1.15] transition-transform duration-300 lg:ml-[30px]  h-[50px] flex flex-col gap-3 lg:flex-row items-center `} onClick={() => setActive("My space")}>
                    <img src={`./images/daredevil.jpeg`} alt="" className={`h-[20px] w-[20px] object-cover rounded-full`} />
            ...
            {['Home', 'Search', "Movies", "Series"].map((item) => (
                <div key={item} className={`${active === item ? 'text-white' : 'text-[#C3C3C3]'} hover:text-white  lg:hover:scale-[1.15] transition-transform duration-300 ${item==="Movies" || item==="Series"?"hidden lg:flex" :'' }  lg:ml-[30px] cursor-pointer   h-[50px] flex flex-col  gap-3 lg:flex-row items-center `}
                 onClick={() => setActive(item)}

                 >
                    <img src={`./images/${active === item ? "white_" + item.toLowerCase() + ".png" : "gray_" + item.toLowerCase() + ".png"}`} alt="" className={ ` h-[20px] w-[20px] object-cover`}
                       />
